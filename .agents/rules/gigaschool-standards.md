...
**使う人から見て何かが変わる直しをしたら、コミットする前に
`docs/CHANGELOG.md` へ 1 行足すこと。**
...
- 日付の見出しは `## YYYY-MM-DD`。**この形でないと機械が拾わず、黙って消える**
- 書いたものは `giga-school.com` のトップ（更新したこと）と紹介ページ・
  使い方マニュアルに、先生と子どもが読む文としてそのまま出る
- 書き方の正本は `giga-changelog` スキル（`.claude/skills/giga-changelog/SKILL.md`）
- 検査: `node .claude/skills/giga-changelog/scripts/lint-changelog.mjs docs/CHANGELOG.md`

⚠️ **コミットの題（`feat:` / `fix:` …）を貼らないこと。** ここに書くのは
「使う人から見て何が変わったか」であって、リポジトリで何をしたかではない。
正本を配る仕組みが同じ `chore(gate)` を 42 本へ撒くので、コミットから作ると
紹介ページに「秘密の直書きの検査を入れる」が並ぶ。使う先生から見れば、
アプリは何も変わっていない。**それを分けられるのは書いた本人だけ。**

⚠️ **使う人から見て何も変わらない直し（CI・検査・道具・正本の同期）には、
書かないこと。** 書く必要が無いときに書くと、更新ログそのものが読まれなくなる。

⚠️ **あとでまとめて書かない。** 直したその場で 1 行足す。あとから書こうとすると
コミットの題を見ながら書くことになり、使う人の言葉に翻訳できなくなる。

Claude Code では `remind-changelog.mjs`（PreToolUse）が、書き忘れたまま
コミットしようとしたときに**1 回だけ**止める。止まったら、書くか、
「使う人から見て何も変わらない」と判断してもう一度同じコマンドを走らせる。
**Antigravity には hook が無いので、この節がそのまま担保になる。**
